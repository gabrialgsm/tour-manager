...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if ($action === 'add') {
            ...
            $q = $db->prepare("UPDATE organization_members SET status=? WHERE id=? AND organization_id=?");
            $q->execute([$status, $memberId, $orgId]);
            if ($status === 'INACTIVE') {

...
$map=[
 'passengers'=>['Passenger List',"SELECT pp.full_name,pp.phone,pp.email,pp.gender,pp.blood_group,tp.departure_from,
COALESCE((SELECT GROUP_CONCAT(DISTINCT b.bus_number ORDER BY b.bus_number SEPARATOR ', ') FROM passenger_seat_assignments psa JOIN buses b ON b.id=psa.bus_id WHERE psa.tour_passenger_id=tp.id),'') bus_number,
COALESCE((SELECT GROUP_CONCAT(DISTINCT s.seat_code ORDER BY s.seat_code SEPARATOR ', ') FROM passenger_seat_assignments psa JOIN seats s ON s.id=psa.seat_id WHERE psa.tour_passenger_id=tp.id),'') seat_code,
COALESCE((SELECT GROUP_CONCAT(DISTINCT r.room_no ORDER BY r.room_no SEPARATOR ', ') FROM room_assignments ra JOIN rooms r ON r.id=ra.room_id WHERE ra.tour_passenger_id=tp.id),'') room_no,
tp.fee,tp.discount,COALESCE((SELECT SUM(amount) FROM payments p WHERE p.tour_passenger_id=tp.id AND p.tour_id=tp.tour_id),0) paid
FROM tour_passengers tp JOIN passenger_profiles pp ON pp.id=tp.passenger_profile_id
WHERE tp.tour_id=? AND tp.status<>'CANCELLED' ORDER BY pp.full_name",
['full_name'=>'Passenger','phone'=>'Phone','email'=>'Email','gender'=>'Gender','blood_group'=>'Blood Group','departure_from'=>'Departure','bus_number'=>'Bus','seat_code'=>'Seat','room_no'=>'Room','fee'=>'Fee (BDT)','discount'=>'Discount (BDT)','paid'=>'Paid (BDT)']],
...
</style></head><body class="export-page"><main class="sheet"><div class="toolbar"><button class="btn primary" onclick="window.print()">Print / Save as PDF</button><a class="btn" href="?type=<?=rawurlencode($type)?>&format=excel">Export Excel</a><a class="btn" id="exportBackBtn" href="dashboard.php">Back</a></div><div class="brand"><div><h1><?=xh($title)?></h1><div class="muted"><?=xh((string)$tour['name'])?></div></div><div class="muted"><?=xh((string)$org['name'])?><br>Generated <?=date('d M Y, h:i A')?><br><?=count($rows)?> records</div></div><table><thead><tr><?php foreach($cols as $label):?><th><?=xh($label)?></th><?php endforeach;?></tr></thead><tbody><?php foreach($rows as $row):?><tr><?php foreach($cols as $key=>$label):?><td><?=xh((string)($row[$key]??''))?></td><?php endforeach;?></tr><?php endforeach;?></tbody></table><div class="footer">GoTM — GoZyraa Tour Management</div></main><script>
(function(){
 const btn=document.getElementById('exportBackBtn');
 if(!btn)return;
 try{
   const ref=document.referrer;
   const u=ref?new URL(ref,location.href):null;
   if(u&&u.origin===location.origin&&!/\/export\.php$/i.test(u.pathname)) btn.href=u.href;
 }catch(e){}
})();
</script></body></html>
...
if ($_SERVER['REQUEST_METHOD']==='POST') {
    try {
        ...
        if($memberId<=0 || !in_array($role,['OWNER','ADMIN','MANAGER','STAFF','VIEWER'],true) || !in_array($status,['ACTIVE','INACTIVE'],true)) throw new RuntimeException('Invalid tour team update.');
        $db->beginTransaction();
        $q=$db->prepare("SELECT om.id,om.user_id,om.role org_role,om.status org_status,u.name,u.username,u.email FROM organization_members om JOIN users u ON u.id=om.user_id WHERE om.id=? AND om.organization_id=? AND om.status='ACTIVE' LIMIT 1 FOR UPDATE");
        $q->execute([$memberId,$orgId]); $member=$q->fetch();
        if(!$member) throw new RuntimeException('Select an active organization member.');
        $q=$db->prepare("SELECT tm.* FROM tour_members tm WHERE tm.tour_id=? AND tm.user_id=? LIMIT 1 FOR UPDATE");
...
if(isset($_POST['action']) && $_POST['action']==='remove') { /* handled below only if no earlier exception */ }

if($_SERVER['REQUEST_METHOD']==='POST' && (string)($_POST['action']??'')==='remove'){
    try{
        saas_check_csrf();
        $memberId=(int)($_POST['member_id']??0);
        $db->beginTransaction();
        $q=$db->prepare("SELECT tm.id,tm.user_id,tm.role,u.name FROM tour_members tm JOIN users u ON u.id=tm.user_id JOIN organization_members om ON om.user_id=tm.user_id AND om.organization_id=? AND om.status='ACTIVE' WHERE tm.id=? AND tm.tour_id=? LIMIT 1 FOR UPDATE");
        $q->execute([$orgId,$memberId,$tourId]);$tm=$q->fetch();
        ...
        $q=$db->prepare("DELETE FROM tour_members WHERE id=? AND tour_id=?");$q->execute([$memberId,$tourId]);
        $db->commit();saas_audit('tour.member_removed','tour_member',$memberId,json_encode(['tour_id'=>$tourId,'user_id'=>(int)$tm['user_id']],JSON_UNESCAPED_UNICODE));$ok='Tour team assignment removed.';
    }catch(Throwable $e){if($db->inTransaction())$db->rollBack();$error=$e->getMessage();}
}

...
if($_SERVER['REQUEST_METHOD']==='POST'){
 try{
  ...
  session_regenerate_id(true);passenger_clear_session();$_SESSION['passenger_account_id']=(int)$a['id'];$_SESSION['passenger_profile_id']=(int)$a['passenger_profile_id'];$_SESSION['passenger_organization_id']=(int)$a['organization_id'];passenger_start_session((int)$a['id']);$db->prepare('UPDATE passenger_accounts SET last_login_at=NOW() WHERE id=?')->execute([(int)$a['id']]);saas_redirect('passenger_dashboard.php');

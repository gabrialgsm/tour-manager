...
if($_SERVER['REQUEST_METHOD']==='POST'){
 try{
  ...
  if($action==='register'){
   $mode='register';$email=strtolower(trim((string)($_POST['email']??'')));$phone=trim((string)($_POST['phone']??''));$password=(string)($_POST['password']??'');
   ...
   $q=$db->prepare("SELECT id,passenger_profile_id,organization_id,password_hash,status FROM passenger_accounts WHERE email=? LIMIT 1");$q->execute([$email]);$existing=$q->fetch();
   if($existing)throw new RuntimeException('An account already exists for this email. Please log in.');
   $q=$db->prepare("SELECT id,organization_id,full_name,phone,email FROM passenger_profiles WHERE email=? ORDER BY id DESC LIMIT 1");$q->execute([$email]);$profile=$q->fetch();
   if(!$profile){
    if($phone==='')throw new RuntimeException('Phone is required when no existing passenger profile matches this email.');
    $org=(int)($_POST['organization_id']??0); if($org<=0)throw new RuntimeException('Use the booking link from your tour to create an account.');
    $q=$db->prepare('INSERT INTO passenger_profiles(organization_id,full_name,phone,email) VALUES(?,?,?,?)');$q->execute([$org,trim((string)($_POST['full_name']??''))?:'Passenger',$phone,$email]);$profile=['id'=>$db->lastInsertId(),'organization_id'=>$org,'phone'=>$phone,'email'=>$email];
   }
   $hash=password_hash($password,PASSWORD_DEFAULT);$q=$db->prepare('INSERT INTO passenger_accounts(passenger_profile_id,organization_id,email,phone,password_hash,email_verified_at) VALUES(?,?,?,?,?,NOW())');$q->execute([(int)$profile['id'],(int)$profile['organization_id'],$email,$phone?:($profile['phone']??null),$hash]);

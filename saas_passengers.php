...
if($_SERVER['REQUEST_METHOD']==='POST'){
    try{
        ...
        $reusedProfile=$profileId>0;

        if($discount>$fee) throw new RuntimeException('Discount cannot exceed fee.');

        $db->beginTransaction();
        if($reusedProfile){
            $q=$db->prepare('SELECT id,full_name,phone,email FROM passenger_profiles WHERE id=? AND organization_id=? FOR UPDATE');
            $q->execute([$profileId,$orgId]);
            if(!$q->fetch()) throw new RuntimeException('Passenger profile not found.');
        }else{
            ...
            $q->execute([$orgId,$name,$phone?:null,$email?:null]);
        ...
        $db->commit();
        saas_audit('passenger.created','tour_passenger',$id,json_encode(['profile_id'=>$profileId,'reused_profile'=>$reusedProfile],JSON_UNESCAPED_UNICODE));
        $ok=$reusedProfile?'Existing passenger profile reused and registered for this tour.':'New passenger registered successfully.';
    }catch(Throwable $e){
